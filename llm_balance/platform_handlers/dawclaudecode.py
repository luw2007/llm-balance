...
class DawClaudeCodeHandler(BasePlatformHandler):
    ...
    def _load_env_config(self):
        ...
        env_user_id = os.getenv('DAWCLAUDECODE_API_USER_ID')
        if env_user_id:
        ...
        config_path = Path.home() / '.llm_balance' / 'dawclaudecode_config.yaml'
    ...
    def get_balance(self) -> CostInfo:
        """Get cost information from DawClaudeCode"""
        api_url = self.config.get('api_url') if isinstance(self.config, dict) else self.config.api_url
        if not api_url:
            raise ValueError("No API URL configured for DawClaudeCode")

        # Get api_user_id from configuration
        api_user_id = getattr(self.config, 'api_user_id', None)
        if not api_user_id:
            raise ValueError(
                "DawClaudeCode api_user_id required. Please set it using one of the following methods:\n"
                "1. Environment variable: export DAWCLAUDECODE_API_USER_ID='your_user_id'\n"
                "2. Config file: llm-balance platform_config dawclaudecode api_user_id 'your_user_id'\n"
                "3. Or create ~/.llm_balance/dawclaudecode_config.yaml with:\n"
        ...
        cookie_domain = getattr(self.config, 'cookie_domain', 'dawclaudecode.com')
        ...
        if not response:
            raise ValueError("No response from DawClaudeCode API")
    ...
    def _extract_balance(self, response: Dict[str, Any]) -> Optional[float]:
        """Extract balance from DawClaudeCode API response
        ...
        try:
            ...
            print(f"Warning: Failed to extract balance from DawClaudeCode response: {e}")
            return None

    def _calculate_spent_amount(self, response: Dict[str, Any]) -> float:
        """Calculate spent amount from DawClaudeCode API response
    ...
    def get_model_tokens(self) -> PlatformTokenInfo:
        ...
        api_user_id = getattr(self.config, 'api_user_id', None)
        if not api_user_id:
            raise ValueError("DawClaudeCode api_user_id required")
