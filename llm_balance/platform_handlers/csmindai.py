"""
CSMindAI third-party relay handler
"""
...
class CSMindAIHandler(RelayPlatformHandler):
    """CSMindAI relay platform handler."""

    platform_id = 'CSMINDAI'
    user_id_header = 'new-api-user'
    quota_scaling = 500000.0

    @classmethod
    def get_default_config(cls) -> dict:
        """Default configuration for CSMindAI."""
        return {
            "display_name": "CSMindAI",
            "handler_class": "CSMindAIHandler",
            "description": "CSMindAI relay (balance and package)",
    ...
    def get_model_tokens(self) -> PlatformTokenInfo:
        ...
        try:
            ...
            remaining = float(data.get('quota', 0))
            used_quota = float(data.get('used_quota', 0))
            total_quota = remaining + used_quota
        except Exception:
            pass

        model_info = ModelTokenInfo(
            model="gpt-4,gpt-3.5-turbo,claude",
            package="CSMindAI Quota Package",
            remaining_tokens=total_quota - used_quota,
